...
fn solve(n: usize, a: &[usize], b: &[usize]) -> String {
    ...
    for i in 0..a.len() {
        max_flow.add_edges(a[i] - 1, b[i] - 1, 1);
    }

    max_flow.dinic(0, n - 1);

    let mut next_queues = vec![VecDeque::new(); n];
    for i in (0..max_flow.edges.len()).step_by(2) {
        if max_flow.edges[i].capacity == 0 {
            next_queues[max_flow.edges[i].from].push_back(max_flow.edges[i].to);
        }
    }
