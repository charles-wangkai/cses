...
fn solve(v: &mut [i32], a: &[usize], b: &[usize], queries: &[String]) -> String {
    ...
    for i in 0..v.len() {
        fenwick_tree.add(in_times[i], v[i]);
    ...
    for query in queries {
        ...
        if kind == 1 {
            ...
            fenwick_tree.add(in_times[s - 1], x - v[s - 1]);
            fenwick_tree.add(out_times[s - 1], v[s - 1] - x);
            v[s - 1] = x;
        } else {
            let s: usize = split.next().unwrap().parse().unwrap();

            result.push(fenwick_tree.compute_prefix_sum(in_times[s - 1]));
        }
    }
