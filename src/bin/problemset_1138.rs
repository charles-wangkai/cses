...
fn solve(v: &mut [i32], a: &[usize], b: &[usize], queries: &[String]) -> String {
    ...
    for i in 0..v.len() {
        update(&mut binary_indexed_tree, in_times[i], v[i]);
    ...
    for query in queries {
        ...
        if kind == 1 {
            ...
            update(&mut binary_indexed_tree, in_times[s - 1], x - v[s - 1]);
            update(&mut binary_indexed_tree, out_times[s - 1], v[s - 1] - x);
            v[s - 1] = x;
        } else {
            let s: usize = split.next().unwrap().parse().unwrap();

            result.push(compute_prefix_sum(&binary_indexed_tree, in_times[s - 1]));
        }
    }
